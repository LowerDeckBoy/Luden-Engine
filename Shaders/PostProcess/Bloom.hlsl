...
void CSMain(uint3 DispatchThreadID : SV_DispatchThreadID)
{
	...
	output[DispatchThreadID.xy].rgb = lightingTexture.Sample(linearClampSampler, texCoord).rgb * Weights[0] * 5.0f;

	for (uint i = 1; i < 5; ++i)
	{
		output[DispatchThreadID.xy].rgb += lightingTexture.Sample(linearClampSampler, texCoord + float2(texOffset.x * i, 0.0)).rgb * Weights[i].x * 5.0f;
		output[DispatchThreadID.xy].rgb += lightingTexture.Sample(linearClampSampler, texCoord - float2(texOffset.x * i, 0.0)).rgb * Weights[i].x * 5.0f;
	}
}
