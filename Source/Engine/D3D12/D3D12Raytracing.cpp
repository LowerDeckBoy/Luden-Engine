...
namespace Luden
{
	...
	void D3D12TLAS::Create()
	{
		...
		Inputs.DescsLayout	= D3D12_ELEMENTS_LAYOUT_ARRAY;
		Inputs.NumDescs		= static_cast<uint32>(Instances.size());

		D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO prebuildInfo{};
		m_RHI->Device->LogicalDevice->GetRaytracingAccelerationStructurePrebuildInfo(&Inputs, &prebuildInfo);

		ScratchSize			= ALIGN(prebuildInfo.ScratchDataSizeInBytes,	D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT);
		ResultSize			= ALIGN(prebuildInfo.ResultDataMaxSizeInBytes,	D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT);
		InstanceDescsSize	= ALIGN(sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * static_cast<uint64>(Instances.size()), D3D12_RAYTRACING_INSTANCE_DESCS_BYTE_ALIGNMENT);
		...
		for (uint32 instanceIdx = 0; instanceIdx < Instances.size(); ++instanceIdx)
		{
			...
			instanceDescs[instanceIdx].InstanceMask = 1;
			instanceDescs[instanceIdx].InstanceID = 0;
		...
		//InstanceBuffer->GetHandle()->Map(0, nullptr, &pData);
		//std::memcpy(pData, instanceDescs.data(), InstanceDescsSize);
		//InstanceBuffer->GetHandle()->Unmap(0, nullptr);

		InstanceBuffer->Create(m_RHI->Device, BufferDesc{
			...
			.Size = InstanceDescsSize
			});

		D3D12UploadContext::UploadBuffer(InstanceBuffer, InstanceDescsSize);
		D3D12UploadContext::Upload();

		Inputs.InstanceDescs = InstanceBuffer->GetGpuAddress();
		...
		m_RHI->GraphicsQueue->Execute({ commandList });
		m_RHI->Wait();
		m_RHI->Flush();

		m_RHI->Device->ShaderResourceHeap->Allocate(AccelerationStructure->ShaderResourceView);
		D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc{};
		srvDesc.Format = DXGI_FORMAT_UNKNOWN;
		srvDesc.ViewDimension = D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE;
		srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
		srvDesc.RaytracingAccelerationStructure.Location = AccelerationStructure->GetGpuAddress();

		m_RHI->Device->LogicalDevice->CreateShaderResourceView(nullptr, &srvDesc, AccelerationStructure->ShaderResourceView.CpuHandle);
	...
	void D3D12TLAS::AddBLAS(Model* pModel)
	{
		...
		blasInput.NumDescs			= static_cast<uint32>(geometryDescs.size());
		blasInput.pGeometryDescs	= geometryDescs.data();

		D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO prebuildInfo{};
		device->LogicalDevice->GetRaytracingAccelerationStructurePrebuildInfo(&blasInput, &prebuildInfo);
		...
		bottomLevelBuildDesc.ScratchAccelerationStructureData = blas->ScratchBuffer->GetGpuAddress();
		bottomLevelBuildDesc.DestAccelerationStructureData = blas->AccelerationStructure->GetGpuAddress();
